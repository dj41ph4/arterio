...
import { CatalogModule } from '../catalog/artwork.module';
...
@Module({
  imports: [PrismaModule, CryptoModule, CatalogModule],
  controllers: [OpenRouterController, AiController, ChatController],
  providers: [
    {
      provide: AI_PROVIDER,
      ...
      useFactory: (config: ConfigService<Env, true>, prisma: PrismaService, crypto: CryptoService) => {
        ...
        const mistralOcrModel = config.get('MISTRAL_OCR_MODEL', { infer: true });
        const mistralVisionModel = config.get('MISTRAL_VISION_MODEL', { infer: true });
        ...
        return new AiProviderChain(
          {
            ...
            mistral: new MistralAiProvider(mistralKey ?? '', mistralModel ?? '', prisma, crypto, {
              ocrModel: mistralOcrModel,
              visionModel: mistralVisionModel,
            }),
          },
