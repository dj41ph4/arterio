...
Return ONLY a JSON object: {"imageUrls": ["...", ...]}`;
  ...
  /** Our provider-agnostic ChatMessage → OpenAI wire shape (identical to Mistral's). */
  ...
  /** ONE tool-calling turn — same wire format as Mistral; tries each configured model in order like completeWithFailover. */
  async chat(input: AiChatInput): Promise<AiChatTurn> {
    const org = await this.resolveOrgSettings(input.organizationId);
    const apiKey = await this.resolveApiKey(org);
    if (!apiKey) {
      throw new ServiceUnavailableException("Aucune clé API OpenRouter configurée — impossible d'appeler l'IA.");
    }
    const attempts: AiAttemptLog[] = [];
    const models = this.resolveModels(org);
    for (const model of models) {
      try {
        ...
        const msg = ok
          ? `Tour de chat OpenRouter réussi (modèle : ${model}${toolCalls.length ? `, ${toolCalls.length} appel(s) d'outil` : ''}).`
          : `Réponse OpenRouter vide (modèle : ${model}).`;
        attempts.push({ model, success: ok, message: msg, provider: 'openrouter' });
        if (!ok) continue;
        return {
          text: toolCalls.length ? undefined : text?.trim(),
          toolCalls: toolCalls.length ? toolCalls : undefined,
          meta: { modelUsed: model, fallbackUsed: attempts.length > 1, attempts, message: msg, hasUsableData: true },
        };
      } catch (e) {
        const message = `Échec du chat OpenRouter ("${model}") : ${describeError(e)}`;
        this.logger.warn(message);
        attempts.push({ model, success: false, message, provider: 'openrouter' });
      }
    }
    throw new ServiceUnavailableException(
      `Tous les modèles OpenRouter ont échoué pour le chat : ${attempts.map((a) => a.message).join(' | ')}`,
    );
  }

  async analyzeImage(_input: AiVisionInput): Promise<AiVisionResult> {
    throw new ServiceUnavailableException("L'analyse d'image n'est pas supportée par le fournisseur OpenRouter.");
  }
