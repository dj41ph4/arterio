...
export class AiProviderChain implements AiProvider {
  ...
  async ocr(input: AiOcrInput): Promise<AiOcrResult> {
    const ordered = await this.resolveOrder(input.organizationId);
    for (const provider of ordered) {
      if (provider.capabilities().ocr && (await provider.isEnabled(input.organizationId))) return provider.ocr(input);
    }
    throw new ServiceUnavailableException('OCR not supported by any configured AI provider');
  }

  /**
   * Pick-first-capable, like describe() — NOT tryInOrder merging: a chat turn's
   * tool-call ids only make sense to the provider that emitted them, so the
   * whole conversation must stay on one provider. The resolved order is stable
   * per org, so successive turns of the same conversation land on the same one.
   */
  async chat(input: AiChatInput): Promise<AiChatTurn> {
    const ordered = await this.resolveOrder(input.organizationId);
    for (const provider of ordered) {
      if (provider.capabilities().chat && (await provider.isEnabled(input.organizationId))) return provider.chat(input);
    }
    throw new ServiceUnavailableException("Aucun fournisseur IA supportant le chat n'est configuré (Réglages → IA).");
  }

  async analyzeImage(input: AiVisionInput): Promise<AiVisionResult> {
    const ordered = await this.resolveOrder(input.organizationId);
    for (const provider of ordered) {
      if (provider.capabilities().vision && (await provider.isEnabled(input.organizationId))) return provider.analyzeImage(input);
    }
