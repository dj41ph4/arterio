...
import { parseColorTarget, paletteMatches, paletteProximity } from '../../common/color-match.util';
...
export class ArtworkService {
  ...
  async list(user: AuthUser, query: ArtworkQuery): Promise<Paginated<ArtworkView>> {
    ...
    // they're not allowed to see. The color filter reads the dominantColors
    // Json column (no SQLite Json filtering in Prisma) — same in-memory path.
    const memoryField = query.sort?.field;
    const colorTarget = query.color ? parseColorTarget(query.color) : null;
    const useMemorySort =
      (memoryField === 'title' || (memoryField === 'value' && canView)) || Boolean(colorTarget);
    ...
    if (useMemorySort) {
      ...
      if (colorTarget) {
        const tol = query.colorTol ?? 35;
        views = views.filter((v) => v.dominantColors.length > 0 && paletteMatches(v.dominantColors, colorTarget, tol));
      }
      if (memoryField === 'title' || (memoryField === 'value' && canView)) {
  ...
  async update(user: AuthUser, id: string, body: Record<string, unknown>): Promise<ArtworkView> {
    ...
    // Same per-locale merge as title — the AI condition report patches one language only.
    if (body.conditionNote !== undefined) {
      data.conditionNote = { ...(existing.conditionNote as Record<string, string>), ...(body.conditionNote as Record<string, string>) };
    }
    ...
    // Invoice-OCR "apply to artwork": purchase price write, gated on the WRITE permission.
    if (valuationPatch?.purchasePrice !== undefined && user.permissions.includes(PERMISSIONS.VALUATION_UPDATE)) {
      const currency = typeof valuationPatch.currency === 'string' && /^[A-Z]{3}$/.test(valuationPatch.currency) ? valuationPatch.currency : undefined;
      await this.prisma.artworkValuation.upsert({
        where: { artworkId: id },
        create: {
          artworkId: id,
          purchasePriceEnc: this.crypto.encryptNumber(valuationPatch.purchasePrice as number | null),
          ...(currency ? { currency } : {}),
        },
        update: {
          purchasePriceEnc: this.crypto.encryptNumber(valuationPatch.purchasePrice as number | null),
  ...
  async autoMergeDuplicates(user: AuthUser): Promise<{
    ...
    for (const members of groups.values()) {
      ...
      const dupIds = members.filter((m) => m.id !== canonical.id).map((d) => d.id);

      await this.mergeGroup(canonical.id, dupIds);
  ...
  /** Reassigns every relation from the duplicates to the canonical artwork, then hard-deletes the duplicates. */
  private async mergeGroup(canonicalId: string, dupIds: string[]): Promise<void> {
    if (!dupIds.length) return;
    await this.prisma.$transaction([
      this.prisma.mediaAsset.updateMany({ where: { artworkId: { in: dupIds } }, data: { artworkId: canonicalId } }),
      this.prisma.document.updateMany({ where: { artworkId: { in: dupIds } }, data: { artworkId: canonicalId } }),
      this.prisma.loanItem.updateMany({ where: { artworkId: { in: dupIds } }, data: { artworkId: canonicalId } }),
      this.prisma.movementRecord.updateMany({ where: { artworkId: { in: dupIds } }, data: { artworkId: canonicalId } }),
      this.prisma.restoration.updateMany({ where: { artworkId: { in: dupIds } }, data: { artworkId: canonicalId } }),
      ...
      ...dupIds.flatMap((dupId) => [
        this.prisma.$executeRaw`
          ...
          SELECT ${canonicalId}, tagId FROM artwork_tags WHERE artworkId = ${dupId}
        `,
        this.prisma.$executeRaw`
          INSERT OR IGNORE INTO exhibition_artworks (exhibitionId, artworkId, wallLabel, sortOrder)
          SELECT exhibitionId, ${canonicalId}, wallLabel, sortOrder FROM exhibition_artworks WHERE artworkId = ${dupId}
        `,
      ]),
  ...
  /**
   * Visual duplicate candidates: artworks whose indexed images have near-identical
   * perceptual hashes (Hamming ≤ threshold of 64 bits — survives recompression,
   * resizes and small crops). Groups are for HUMAN review — nothing is merged here.
   */
  async findVisualDuplicates(user: AuthUser): Promise<{
    groups: Array<{ artworks: ArtworkView[]; similarity: number }>;
    comparedImages: number;
  }> {
    const HAMMING_THRESHOLD = 10;
    const assets = await this.prisma.mediaAsset.findMany({
      where: { organizationId: user.organizationId, phash: { not: null }, artworkId: { not: null }, artwork: { deletedAt: null } },
      select: { artworkId: true, phash: true },
    });
    ...
    // Union-find over artworks linked by at least one near-identical image pair.
    ...
    for (const a of assets) parent.set(a.artworkId!, a.artworkId!);
    ...
    for (let i = 0; i < assets.length; i++) {
      for (let j = i + 1; j < assets.length; j++) {
        const a = assets[i]!;
        const b = assets[j]!;
        if (a.artworkId === b.artworkId) continue;
        const d = hamming(a.phash!, b.phash!);
        if (d <= HAMMING_THRESHOLD) {
          union(a.artworkId!, b.artworkId!);
          const key = [a.artworkId, b.artworkId].sort().join('|');
          bestDistance.set(key, Math.min(bestDistance.get(key) ?? 64, d));
        }
      }
    }
    ...
    const canView = this.canViewValuation(user);
    const groups: Array<{ artworks: ArtworkView[]; similarity: number }> = [];
    for (const members of clusters.values()) {
      if (members.size < 2) continue;
      const rows = await this.prisma.artwork.findMany({
        where: { id: { in: [...members] }, deletedAt: null },
        include: ARTWORK_INCLUDE,
      });
      ...
      for (let i = 0; i < ids.length; i++) {
        for (let j = i + 1; j < ids.length; j++) {
          ...
          minDistance = Math.min(minDistance, bestDistance.get(key) ?? 64);
        }
      }
      groups.push({
        artworks: rows.map((r) => toArtworkView(r as never, { crypto: this.crypto, canViewValuation: canView })),
        similarity: Math.round((1 - minDistance / 64) * 100),
      });
    }
    groups.sort((a, b) => b.similarity - a.similarity);
    return { groups, comparedImages: assets.length };
  }

  /** User-confirmed merge from the visual-duplicates review: keep `canonicalId`, fold the rest into it. */
  async mergeArtworks(user: AuthUser, canonicalId: string, duplicateIds: string[]): Promise<void> {
    const ids = [canonicalId, ...duplicateIds];
    const found = await this.prisma.artwork.count({ where: { id: { in: ids }, organizationId: user.organizationId, deletedAt: null } });
    if (found !== ids.length) throw new NotFoundException('Artwork not found');
    if (duplicateIds.includes(canonicalId)) throw new BadRequestException('canonicalId cannot be in duplicateIds');
    await this.mergeGroup(canonicalId, duplicateIds);
    await this.audit.log({
      organizationId: user.organizationId,
      actorId: user.sub,
      action: 'artwork.merge',
      resource: 'artwork',
  ...
  /**
   * "Œuvres similaires" for the detail page — weighted blend of visual
   * signature (phash + palette) and catalogue metadata. Degrades gracefully:
   * with no indexed images the metadata terms still rank same-artist /
   * same-technique works, so the strip works before any backfill has run.
   */
  async findSimilar(user: AuthUser, id: string, limit = 8): Promise<ArtworkView[]> {
    const anchor = await this.prisma.artwork.findFirst({
      where: { id, organizationId: user.organizationId, deletedAt: null },
      include: { ...ARTWORK_INCLUDE, media: { orderBy: { sortOrder: 'asc' as const }, select: { id: true, storageKey: true, phash: true } } },
    });
    if (!anchor) throw new NotFoundException('Artwork not found');

    const candidates = await this.prisma.artwork.findMany({
    ...
    const anchorHashes = (anchor.media as Array<{ phash: string | null }>).map((m) => m.phash).filter((p): p is string => !!p);
    const anchorColors = (anchor.dominantColors as string[] | null) ?? [];

    const scored = candidates
      .map((c) => {
        let score = 0;
        const cHashes = (c.media as Array<{ phash: string | null }>).map((m) => m.phash).filter((p): p is string => !!p);
        if (anchorHashes.length && cHashes.length) {
          let best = 64;
          for (const a of anchorHashes) for (const b of cHashes) best = Math.min(best, hamming(a, b));
          score += 0.35 * (1 - best / 64);
        }
        const cColors = (c.dominantColors as string[] | null) ?? [];
        if (anchorColors.length && cColors.length) {
          score += 0.25 * paletteProximity(anchorColors, cColors);
        }
        if (anchor.artistId && c.artistId === anchor.artistId) score += 0.2;
        if (anchor.techniqueId && c.techniqueId === anchor.techniqueId) score += 0.1;
        if ((anchor.movementId && c.movementId === anchor.movementId) || (anchor.categoryId && c.categoryId === anchor.categoryId)) score += 0.1;
        return { c, score };
      })
    ...
    const canView = this.canViewValuation(user);
    return scored.map((s) => toArtworkView(s.c as never, { crypto: this.crypto, canViewValuation: canView }));
  }
