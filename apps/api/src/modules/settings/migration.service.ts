...
export class MigrationService {
  ...
  /**
   * Restore-in-place: wipes all content of the current org and re-imports from the ZIP,
   * keeping the same org ID and user accounts so the caller stays logged in.
   * Users from the archive are merged by email — existing accounts are untouched,
   * new ones are created.
   */
  async restoreMigration(orgId: string, zipPath: string): Promise<{ restoredItems: number }> {
    const zip = new AdmZip(zipPath);
    const dataEntry = zip.getEntry('data.json');
    if (!dataEntry) throw new Error('Invalid migration file: data.json not found');
    const data = JSON.parse(zip.readAsText(dataEntry));
    ...
    await this.prisma.$transaction(
      async (tx) => {
        // ── 1. Wipe all content for this org in dependency order ──────────────
        await tx.exhibitionArtwork.deleteMany({ where: { exhibition: { organizationId: orgId } } });
        await tx.exhibition.deleteMany({ where: { organizationId: orgId } });
        await tx.documentSignature.deleteMany({ where: { document: { organizationId: orgId } } });
        await tx.documentVersion.deleteMany({ where: { document: { organizationId: orgId } } });
        await tx.document.deleteMany({ where: { organizationId: orgId } });
        await tx.loanItem.deleteMany({ where: { loan: { organizationId: orgId } } });
        await tx.transport.deleteMany({ where: { loan: { organizationId: orgId } } });
        await tx.insurancePolicy.deleteMany({ where: { loan: { organizationId: orgId } } });
        await tx.loan.deleteMany({ where: { organizationId: orgId } });
        await tx.movementRecord.deleteMany({ where: { artwork: { organizationId: orgId } } });
        await tx.restoration.deleteMany({ where: { organizationId: orgId } });
        await tx.mediaAsset.deleteMany({ where: { organizationId: orgId } });
        await tx.artworkTag.deleteMany({ where: { artwork: { organizationId: orgId } } });
        await tx.artworkValuation.deleteMany({ where: { artwork: { organizationId: orgId } } });
        await tx.artwork.deleteMany({ where: { organizationId: orgId } });
        await tx.artist.deleteMany({ where: { organizationId: orgId } });
        await tx.artMovement.deleteMany({ where: { organizationId: orgId } });
        // null out parentId before deleting self-referential collections / locations
        await tx.collection.updateMany({ where: { organizationId: orgId }, data: { parentId: null } });
        await tx.collection.deleteMany({ where: { organizationId: orgId } });
        await tx.category.deleteMany({ where: { organizationId: orgId } });
        await tx.technique.deleteMany({ where: { organizationId: orgId } });
        await tx.support.deleteMany({ where: { organizationId: orgId } });
        ...
        // ── 3. Re-insert all content (same logic as importMigration) ─────────
        for (const m of data.movements) {
          const created = await tx.artMovement.create({ data: { organizationId: orgId, name: m.name, label: m.label } });
          ids.movement.set(m.id, created.id);
        }
        for (const c of data.collections) {
          const created = await tx.collection.create({
            data: { organizationId: orgId, name: c.name, label: c.label, description: c.description, color: c.color },
          });
          ids.collection.set(c.id, created.id);
        }
        for (const c of data.collections) {
          if (c.parentId) await tx.collection.update({ where: { id: ids.collection.get(c.id) }, data: { parentId: remap(ids.collection, c.parentId) } });
        }
        for (const c of data.categories) {
          const created = await tx.category.create({ data: { organizationId: orgId, name: c.name, label: c.label, color: c.color } });
          ids.category.set(c.id, created.id);
        }
        for (const t of data.techniques) {
          const created = await tx.technique.create({ data: { organizationId: orgId, name: t.name, label: t.label } });
          ids.technique.set(t.id, created.id);
        }
        for (const s of data.supports) {
          const created = await tx.support.create({ data: { organizationId: orgId, name: s.name, label: s.label } });
          ids.support.set(s.id, created.id);
        }
        for (const t of data.tags) {
          const created = await tx.tag.create({ data: { organizationId: orgId, name: t.name, color: t.color, aiGenerated: t.aiGenerated } });
          ids.tag.set(t.id, created.id);
        }
        for (const a of data.artists) {
          const created = await tx.artist.create({
            data: {
              ...
              fullName: a.fullName, sortName: a.sortName, nationality: a.nationality,
              birthDate: a.birthDate, deathDate: a.deathDate, biography: a.biography,
              movementId: remap(ids.movement, a.movementId), externalIds: a.externalIds,
              thumbnail: a.thumbnail, notableWorks: a.notableWorks, influencedBy: a.influencedBy,
            },
          });
          ids.artist.set(a.id, created.id);
        }
        for (const a of data.artworks) {
          const created = await tx.artwork.create({
            data: {
              organizationId: orgId,
              inventoryNumber: a.inventoryNumber, accessionNumber: a.accessionNumber,
              title: a.title, description: a.description, analysis: a.analysis,
              notes: a.notes, artistId: remap(ids.artist, a.artistId),
              attribution: a.attribution, authentication: a.authentication,
              signature: a.signature, signatureDescription: a.signatureDescription,
              movementId: remap(ids.movement, a.movementId), categoryId: remap(ids.category, a.categoryId),
              techniqueId: remap(ids.technique, a.techniqueId), supportId: remap(ids.support, a.supportId),
              dateText: a.dateText, yearFrom: a.yearFrom, yearTo: a.yearTo,
              heightCm: a.heightCm, widthCm: a.widthCm, depthCm: a.depthCm,
              weightKg: a.weightKg, dimensionsNote: a.dimensionsNote,
              framed: a.framed, frameNote: a.frameNote, dominantColors: a.dominantColors,
              status: a.status, condition: a.condition, conditionNote: a.conditionNote,
              acquisitionMethod: a.acquisitionMethod, acquisitionDate: a.acquisitionDate,
              paymentMethod: a.paymentMethod, ownerName: a.ownerName,
              collectionId: remap(ids.collection, a.collectionId),
              hasCertificate: a.hasCertificate, hasInvoice: a.hasInvoice,
              barcode: a.barcode, rfidTag: a.rfidTag, nfcTag: a.nfcTag,
              provenance: a.provenance, bibliography: a.bibliography,
              references: a.references, externalLinks: a.externalLinks,
              aiMeta: a.aiMeta, isFavorite: a.isFavorite,
            },
          });
          ids.artwork.set(a.id, created.id);
        }
        ...
        for (const v of data.valuations) {
          ...
          await tx.artworkValuation.create({
            data: {
              artworkId, currency: v.currency,
              purchasePriceEnc: this.crypto.encryptNumber(v.purchasePrice),
              currentValueEnc: this.crypto.encryptNumber(v.currentValue),
              insuranceValueEnc: this.crypto.encryptNumber(v.insuranceValue),
              valuationDate: v.valuationDate, valuationSource: v.valuationSource,
            },
          });
        }
        for (const l of data.locations) {
          const created = await tx.location.create({ data: { organizationId: orgId, kind: l.kind, name: l.name, code: l.code, mapMeta: l.mapMeta } });
          ids.location.set(l.id, created.id);
        }
        for (const l of data.locations) {
          if (l.parentId) await tx.location.update({ where: { id: ids.location.get(l.id) }, data: { parentId: remap(ids.location, l.parentId) } });
        }
        for (const m of data.media) {
          const artworkId = remap(ids.artwork, m.artworkId);
          const created = await tx.mediaAsset.create({
            data: {
              organizationId: orgId, artworkId, type: m.type, role: m.role,
              storageKey: m.storageKey, derivatives: m.derivatives, fileName: m.fileName,
              mimeType: m.mimeType, sizeBytes: m.sizeBytes, width: m.width, height: m.height,
              checksum: m.checksum, exif: m.exif, phash: m.phash, caption: m.caption,
              sortOrder: m.sortOrder, encrypted: m.encrypted,
            },
          });
          ids.media.set(m.id, created.id);
        }
        for (const mr of data.movementRecords) {
          ...
          await tx.movementRecord.create({
            data: {
              artworkId, fromId: remap(ids.location, mr.fromId), toId: remap(ids.location, mr.toId),
              reason: mr.reason, movedById: null, movedAt: mr.movedAt,
            },
          });
        }
        for (const d of data.documents) {
          const created = await tx.document.create({
            data: {
              organizationId: orgId, artworkId: remap(ids.artwork, d.artworkId),
              type: d.type, title: d.title, ocrText: d.ocrText, encrypted: d.encrypted,
            },
          });
          ids.document.set(d.id, created.id);
        }
        for (const v of data.documentVersions) {
          ...
          if (!documentId) continue;
          await tx.documentVersion.create({
            data: { documentId, version: v.version, storageKey: v.storageKey, mimeType: v.mimeType, sizeBytes: v.sizeBytes, checksum: v.checksum, note: v.note, createdById: null },
          });
        }
        for (const l of data.loans) {
          const created = await tx.loan.create({
            data: {
              organizationId: orgId, reference: l.reference, direction: l.direction,
              status: l.status, counterparty: l.counterparty, contactInfo: l.contactInfo,
              startDate: l.startDate, endDate: l.endDate, returnedAt: l.returnedAt, conditions: l.conditions,
            },
          });
          ids.loan.set(l.id, created.id);
        }
        for (const li of data.loanItems) {
          const loanId = remap(ids.loan, li.loanId);
          const artworkId = remap(ids.artwork, li.artworkId);
          if (loanId && artworkId) await tx.loanItem.create({ data: { loanId, artworkId, conditionOut: li.conditionOut, conditionIn: li.conditionIn } });
        }
        for (const t of data.transports) {
          await tx.transport.create({
            data: {
              loanId: remap(ids.loan, t.loanId), carrier: t.carrier, method: t.method,
              trackingNumber: t.trackingNumber, departureAt: t.departureAt, arrivalAt: t.arrivalAt,
              cost: t.cost, currency: t.currency, notes: t.notes,
            },
          });
        }
        for (const ip of data.insurancePolicies) {
          await tx.insurancePolicy.create({
            data: {
              loanId: remap(ids.loan, ip.loanId), insurer: ip.insurer, policyNumber: ip.policyNumber,
              coverageEnc: this.crypto.encryptNumber(ip.coverage), currency: ip.currency,
              startDate: ip.startDate, endDate: ip.endDate, notes: ip.notes,
            },
          });
        }
        for (const e of data.exhibitions) {
          const created = await tx.exhibition.create({
            data: {
              organizationId: orgId, title: e.title, kind: e.kind, status: e.status,
              description: e.description, venue: e.venue, startDate: e.startDate, endDate: e.endDate, curator: e.curator,
            },
          });
          ids.exhibition.set(e.id, created.id);
        }
        for (const ea of data.exhibitionArtworks) {
          const exhibitionId = remap(ids.exhibition, ea.exhibitionId);
          const artworkId = remap(ids.artwork, ea.artworkId);
          if (exhibitionId && artworkId) await tx.exhibitionArtwork.create({ data: { exhibitionId, artworkId, wallLabel: ea.wallLabel, sortOrder: ea.sortOrder } });
        }
        for (const r of data.restorations) {
          const artworkId = remap(ids.artwork, r.artworkId);
          if (!artworkId) continue;
          await tx.restoration.create({
            data: {
              organizationId: orgId, artworkId, status: r.status, title: r.title,
              diagnosis: r.diagnosis, treatment: r.treatment, conservator: r.conservator,
              costEnc: this.crypto.encryptNumber(r.cost), currency: r.currency,
              startDate: r.startDate, endDate: r.endDate,
              beforeMediaIds: r.beforeMediaIds, afterMediaIds: r.afterMediaIds,
            },
          });
        }
