...
function matches(a: ArtworkView, q: ArtworkQuery): boolean {
  ...
  if (q.color && !paletteMatchesColor(a.dominantColors, q.color, q.colorTol ?? 35)) return false;
  return true;
}

/** Compact mirror of the API's hue matcher (common/color-match.util.ts) so the color filter behaves in mock mode too. */
function paletteMatchesColor(palette: string[], color: string, tol: number): boolean {
  const toHsl = (hex: string): { h: number; s: number; l: number } | null => {
  ...
  const target = color === 'black' || color === 'white' || color === 'gray' ? color : toHsl(color);
  if (!target) return false;
  for (const hex of palette) {
    const c = toHsl(hex);
    if (!c) continue;
    if (target === 'black') { if (c.l < 0.18) return true; continue; }
    if (target === 'white') { if (c.l > 0.82) return true; continue; }
    if (target === 'gray') { if (c.s < 0.12 && c.l >= 0.18 && c.l <= 0.82) return true; continue; }
    if (c.s >= 0.12) {
      const d = Math.abs(c.h - target.h) % 360;
      if ((d > 180 ? 360 - d : d) <= tol) return true;
    }
  }
  return false;
}
