...
export const aiApi = {
  ...
  /** « Parle à ta collection » — one grounded answer per call; the server runs the whole tool loop. */
  chat: (input: { messages: ChatMessage[]; locale: Locale }) =>
    apiFetch<ChatResponse>('/ai/chat', { method: 'POST', body: JSON.stringify(input) }),
};
