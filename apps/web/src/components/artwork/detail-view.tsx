...
import { DocumentOcrActions } from '@/components/documents/document-ocr-actions';
...
import { SimilarWorksStrip } from './similar-works-strip';
...
export function ArtworkDetailView({ id }: { id: string }) {
  ...
  const qrUrl = typeof window !== 'undefined'
    ? `${window.location.origin}/${locale}/artworks/${art.id}`
    : `/${locale}/artworks/${art.id}`;
  ...
  return (
    <div className="mx-auto max-w-[1400px] p-4 md:p-6 lg:p-8">
      ...
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            variant="outline"
            size="icon-sm"
            onClick={() => { navigator.clipboard.writeText(qrUrl); toast.success(t('artwork.share.copied')); }}
          >
            <Share2 className="size-4" />
          </Button>
          <Button variant="outline" size="icon-sm" onClick={() => setQrOpen(true)}>
        ...
        <div className="min-w-0">
          ...
          <Tabs defaultValue="overview" className="mt-6">
            ...
            <TabsContent value="overview">
              ...
              <Separator className="my-4" />
              <SimilarWorksStrip artworkId={art.id} />
            </TabsContent>
            ...
            <TabsContent value="conservation">
              <ArtworkConservationTab artworkId={art.id} hasMedia={art.media.length > 0} />
            </TabsContent>
      ...
      <QrModal open={qrOpen} onClose={() => setQrOpen(false)} title={title} url={qrUrl} />
    </div>
  );
}
...
interface ArtworkDocumentRow {
  ...
  hasFile?: boolean;
  hasOcr?: boolean;
  extractedFields?: { price?: number; currency?: string; date?: string; seller?: string; invoiceNumber?: string } | null;
}
...
function ArtworkConservationTab({ artworkId, hasMedia }: { artworkId: string; hasMedia: boolean }) {
  const t = useTranslations();
  const locale = useLocale() as Locale;
  const qc = useQueryClient();
  ...
  const [suggestion, setSuggestion] = React.useState<ConditionSuggestion | null>(null);
  ...
  const reportMutation = useMutation({
    mutationFn: () =>
      apiFetch<ConditionSuggestion & { modelUsed: string | null }>('/ai/condition-report', {
        method: 'POST',
        body: JSON.stringify({ artworkId, locale }),
      }),
    onSuccess: (s) => {
      setSuggestion(s);
      setEditedNote(s.note);
    },
    onError: (err) => toast.error(err instanceof Error ? err.message : t('artwork.conditionAi.failed')),
  });

  const acceptMutation = useMutation({
    mutationFn: () =>
      apiFetch(`/artworks/${artworkId}`, {
        method: 'PATCH',
        // conditionNote merges per locale server-side — other languages stay untouched.
        body: JSON.stringify({ condition: suggestion!.condition, conditionNote: { [locale]: editedNote } }),
      }),
    onSuccess: () => {
      toast.success(t('artwork.conditionAi.acceptedToast'));
      setSuggestion(null);
      invalidate();
    },
    onError: (err) => toast.error(err instanceof Error ? err.message : t('artwork.conditionAi.failed')),
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-end gap-2">
        <Button
          ...
          disabled={!hasMedia || reportMutation.isPending}
          title={!hasMedia ? t('artwork.conditionAi.noMedia') : undefined}
          className="gap-1.5"
        >
          <Sparkles className={cn('size-3.5', reportMutation.isPending && 'animate-pulse')} />
          {reportMutation.isPending ? t('artwork.conditionAi.generating') : t('artwork.conditionAi.generate')}
        </Button>
      ...
      {suggestion && (
        <div className="space-y-2.5 rounded-xl border border-primary/25 bg-primary/5 p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-sm font-medium">
              <Sparkles className="size-4 text-primary" />
              {t('artwork.conditionAi.suggestion')}
            </div>
            <ConditionBadge condition={suggestion.condition as never} />
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            <Button size="sm" onClick={() => acceptMutation.mutate()} disabled={acceptMutation.isPending} className="gap-1.5">
              <Check className="size-3.5" /> {t('artwork.conditionAi.accept')}
            </Button>
            <Button
              size="sm"
              variant="outline"
              onClick={() => {
                setFormOpen(true);
                setDiagnosis(editedNote);
                setSuggestion(null);
              }}
              className="gap-1.5"
            >
              <Wrench className="size-3.5" /> {t('artwork.conditionAi.prefillRestoration')}
            </Button>
...
function MediaTab({
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => artworkRepository.uploadMedia(artworkId, await compressImage(file)),
    onSuccess: () => {
