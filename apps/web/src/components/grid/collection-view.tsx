...
import { Check, Star, Pencil, Trash2, X, LibraryBig, Upload, Plus, Merge, Sparkles, Globe, ScanSearch } from 'lucide-react';
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import { aiApi } from '@/lib/data/ai';
import { mediaIndexApi } from '@/lib/data/media-index';
...
import { VisualDuplicatesModal } from './visual-duplicates-modal';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function CollectionView({ favoritesOnly = false }: { favoritesOnly?: boolean }) {
  ...
  // Also seeded from URL params so the AI "smart search" (command palette) can
  // land on a pre-filtered collection: /collection?q=…&status=…&color=…
  ...
  const [colorFilter, setColorFilter] = React.useState<string | null>(() => searchParams.get('color'));
  ...
  // Table/spreadsheet need ~1700px — on a phone the gallery is the only usable mode.
  const isMobile = useIsMobile();
  const effectiveViewMode: ViewMode = isMobile ? 'gallery' : viewMode;
  const [importOpen, setImportOpen] = React.useState(false);
  const [visualDupesOpen, setVisualDupesOpen] = React.useState(false);
  ...
  // Visual indexing backfill (phash + couleurs) — same job/poll pattern as bulk autofill
  const { data: indexStatus, refetch: refetchIndexStatus } = useQuery({
    queryKey: ['media-index-status'],
    queryFn: mediaIndexApi.status,
    refetchInterval: (q) => (q.state.data?.running ? 1500 : false),
    staleTime: 0,
  });
  const wasIndexRunning = React.useRef(false);
  React.useEffect(() => {
    if (!indexStatus) return;
    if (wasIndexRunning.current && !indexStatus.running) {
      qc.invalidateQueries({ queryKey: ['artworks'] });
      toast.success(t('grid.mediaIndex.done', { indexed: indexStatus.indexed, total: indexStatus.total }));
    }
    wasIndexRunning.current = !!indexStatus.running;
  }, [indexStatus, qc, t]);
  ...
  const query: Omit<ArtworkQuery, 'cursor' | 'limit'> = React.useMemo(
    ...
    [search, statusFilter, collectionFilter, colorFilter, exhibitionId, locationId, sorting, favoritesOnly, locale],
  );
  ...
  return (
    <div className="flex h-[calc(100dvh-4rem)] flex-col">
            ...
            <div className="flex items-center gap-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start">
                  <DropdownMenuItem onSelect={() => dedupMutation.mutate()}>
                    {t('duplicates.autoByTitle')}
                  </DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setVisualDupesOpen(true)}>
                    {t('duplicates.visualAction')}
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => mediaIndexApi.start().then(() => refetchIndexStatus()).catch(() => toast.error(t('grid.mediaIndex.failed')))}
                disabled={indexStatus?.running}
                className="flex items-center gap-2"
                title={t('grid.mediaIndex.hint')}
              >
                <ScanSearch className={cn('h-4 w-4', indexStatus?.running && 'animate-pulse')} />
                {indexStatus?.running
                  ? t('grid.mediaIndex.running', { done: indexStatus.done, total: indexStatus.total })
                  : t('grid.mediaIndex.start')}
              </Button>
              <Button
                variant="outline"
                size="sm"
      ...
      <VisualDuplicatesModal open={visualDupesOpen} onClose={() => setVisualDupesOpen(false)} />
