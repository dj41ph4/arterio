...
| `MISTRAL_MODEL` | `mistral-medium-latest` | used for plain completions, the websearch agent, AND the « Parle à ta collection » chat (tool-calling) |
| `MISTRAL_OCR_MODEL` | `mistral-ocr-latest` | dedicated OCR model — invoice scanning (`POST /documents/:id/ocr`) |
| `MISTRAL_VISION_MODEL` | `pixtral-12b-2409` | vision model — AI condition report (`POST /ai/condition-report`) |
